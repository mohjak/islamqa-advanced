...
class CreateFatwa implements Hookable {
    use UpsertFatwa;
    ...
    public function register_input_fields(): void  {
        $input_type = 'Create' . FatwaPostType::GRAPHQL_SINGLE_NAME . 'Input';
    ...
    public function validate( $source, array $args, AppContext $context, ResolveInfo $info, $field_resolver, string $type_name, string $field_key ): void {
        if ( 'RootMutation' !== $type_name || ! $this->is_create_fatwa_mutation( $field_key ) ) {
            return;
        }
    ...
    public function save_additional_data( int $post_id, array $input, WP_Post_Type $post_type_object, string $mutation_name ): void {
        if ( ! $this->is_create_fatwa_mutation( $mutation_name ) ) {
            return;
        }
    ...
    private function is_create_fatwa_mutation( string $mutation_name ): bool {
        return 'create' . FatwaPostType::GRAPHQL_SINGLE_NAME === $mutation_name;
    }
}
